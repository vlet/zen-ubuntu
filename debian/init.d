...
sub runhttpsgui()
{
	...
	{
		system ( "/usr/sbin/mini-httpd -C $confhttp > /dev/null &" );
		exit ( 0 );
	}
}
...
if ( $eject eq "start" )
{
	...
	if ( -e $filecluster )
	{
		...
		if ( $clstatus =~ /UP/ )
		{
			...
			# emit net-device-up event
			`$initctl emit --no-wait net-device-up IFACE="@cliface[2]"`;
	...
	foreach $file ( @files )
	{
		...
		if ( $file !~ /:/ && $file !~ /\./ )
		{
			...
			if ( @lastline[4] eq "up" )
			{
				...
				{
					# emit net-device-up event
					`$initctl emit --no-wait net-device-up IFACE="@lastline[0]"`;
					print " \033[1;32m OK \033[0m \n";
				}
				else
				{
